...
import { Row, Col, Form, Button, Input, Typography, Rate, Modal, Avatar, Skeleton, Upload, message, Tooltip } from "antd";
import {
    UploadOutlined
    ...
    const propsFile = {
        ...
        beforeUpload: file => {
            if (!types.includes(file.type)) {
                message.error(`El archivo ${file.name}, esta en un formato no permitido, 
           toma en cuenta que no sera subido`, 10);
                return false;
            } else {
                return true
            }
                ...
                profile == "" ?
                    ...
                    <Row gutter={[12, 12]}>
                        <Col xs={24} md={12}>
                            <Row gutter={[12, 12]}>
                                <Col xs={0} sm={24} style={{ alignItems: 'center' }}>
                                    <Avatar src={`https://api.epno-app.com${profile.logo}`} size={300} shape="square" />
                                </Col>
                        ...
                        <Col xs={24} md={12}>
                            ...
                            <Row gutter={[12, 12]}>
                                <Col xs={24} md={10}>
                                    <Text>Tel:<Title level={4}>{profile.phone}</Title></Text>
                                </Col>
                                <Col xs={24} md={14}>
                                ...
                                <Col xs={24} md={14}>
                                    {
                                        (profile.role_id == 1 || profile.role_id == 2 || profile.role_id == 3 || profile.role_id == 5) &&
                                        <Text>Org rate:<Title level={3}><Rate tooltips={desc} value={profile.org_rate} allowClear={false} allowHalf disabled /></Title></Text>
                                    }
                                </Col>
